...
class ApiController extends Controller {
	...
	public function getEvents(Request $request) {
		...
		if (!empty($request->radius)) {
			$events = Event::getEventsByRadius($request->latLng, $request->radius, $request->type, $offset, 50, false);
		} else {
			$events = Event::getEvents($request->city, $request->type, $offset, false);
		}
		$featured_events = Event::getEvents('', '', 0, true);
		...
		if (count($events) > 0 || count($featured_events) > 0) {
			...
			foreach ($featured_events as $key => $event) {
				...
				$category = DB::table('categories')->where('id', $event->category_id)->first();
				if ($event->user_id != 0) {
					$user                               = DB::table('users')->select('username', 'email', 'is_verified')->where('id', $events->user_id)->first();
	...
	public function saveFavourite(Request $request) {
		...
		$data = ['user_id' => $request->user_id, 'event_id' => $request->event_id, 'created_at' => $this->current_date_time];
	...
	public function eventShare(Request $request) {
		...
		$res = DB::table('events')->where('id', '=', $request->event_id)->increment('share_count');
	...
	public function register(Request $request) {
		...
		return array($destinationPath.$picture, $destinationPath);
		}*/
		} else {
			$picture = 'default.png';
		}
	...
	public function updateProfile(Request $request) {
		...
		if ($check) {
			$res = DB::table('users')->where('email', '=', $email)->update(['gender' => $request->gender, 'dob' => $request->dob]);
